import { NextRequest, NextResponse } from "next/server";
import prisma from "@/lib/prisma";

export const dynamic = "force-dynamic";

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();

    const escrowId = body.escrowId || `ESCROW-${Date.now()}`;
    const consumer = body.consumerUid || body.consumerAddress || body.consumer;
    const provider = body.providerId || body.providerUid || body.providerAddress || body.provider;
    const rawAmount = body.amount ?? body.amountPi;
    const numericAmount = Number(rawAmount);

    if (!consumer || !provider || isNaN(numericAmount) || numericAmount <= 0) {
      return NextResponse.json(
        {
          success: false,
          error: "MISSING_FIELDS: Required parameters missing or invalid.",
          received: { escrowId, consumer, provider, amount: numericAmount }
        },
        { status: 400 }
      );
    }

    const db = prisma as any;

    // Check if provider exists or find first available provider in DB
    let validProviderId = provider;
    try {
      const existingProvider = await db.serviceProvider.findFirst();
      if (existingProvider) {
        validProviderId = existingProvider.id;
      }
    } catch (e) {
      // If ServiceProvider table is empty or unseeded
    }

    const escrowPayload: any = {
      escrowId,
      consumerUid: consumer,
      amount: numericAmount,
      token: body.token || "PI",
      status: "LOCKED",
      timelockExpiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
      serviceDescription: body.serviceDescription || "Canary Escrow Lock",
      contractId: body.contractId || process.env.NEXT_PUBLIC_SOROBAN_VAULT_CONTRACT_ID || "CB5CQFNEPLQRZGNWXIXOXEK4L2LPYUJ3QCCCVHVKE5CSZFJXZ2HZQHIQ",
      sorobanTxHash: body.sorobanTxHash || `soroban_hold_${Math.random().toString(36).substring(2, 12)}`,
      originNode: body.originNode || "Node-001-X570-Taichi",
      network: process.env.NEXT_PUBLIC_SOROBAN_NETWORK || "Soroban-Testnet",
    };

    if (validProviderId && /^[0-9a-fA-F]{24}$/.test(validProviderId)) {
      escrowPayload.providerId = validProviderId;
    }

    const escrowRecord = await db.escrowLock.create({
      data: escrowPayload,
    });

    return NextResponse.json(
      {
        success: true,
        message: "Escrow locked successfully.",
        escrowId,
        escrow: escrowRecord,
      },
      { status: 201 }
    );
  } catch (error: any) {
    console.error("[API_ESCROW_LOCK_ERROR]:", error);
    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Failed to lock escrow.",
        meta: error?.meta || null
      },
      { status: 500 }
    );
  }
}
