...
.SYNOPSIS
    ...
    PowerShell Passport API Verification Script (v1.0.0)
.DESCRIPTION
    ...
    '/api/mesh/passport-verify' endpoint. Emulates mobile network handshakes
    and checks JSON parsing integrity under sandbox constraints.
.NOTES
...
function Test-Handshake {
    ...
    try {
        ...
        # We use Invoke-WebRequest to inspect headers and prevent automatic throwing on bad status codes
        $Response = Invoke-WebRequest -Uri $FullUrl -Method Get -UseBasicParsing -ErrorAction SilentlyContinue -TimeoutSec 10
        $Stopwatch.Stop()
        ...
        if ($null -eq $Response) {
            throw "No response returned from network interface."
        }
        ...
        if ($StatusCode -eq 200) {
            ...
            try {
                ...
                Write-Host "  --> Tier Level: ${Cyan}$($Payload.tierLevel)${Reset} | Issued: $($Payload.isIssued) | Source: $($Payload.source)"
                if ($Detailed) {
            ...
            if ($StatusCode -eq 400 -and $QueryString -not-match "wallet") {
        ...
        Write-Host "${Red}  [!] CONNECTION CRASH: Failed to communicate with the development server.${Reset}"
