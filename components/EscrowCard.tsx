// Location: components/EscrowCard.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { useBazaarVault } from '@/hooks/useBazaarVault';
import { EscrowStatus } from '@/types/bazaar-vault';
import { ShieldCheck, Lock, AlertCircle, RefreshCw, Scale } from 'lucide-react';
...
export function EscrowCard({
  ...
  defaultTokenContract = process.env.NEXT_PUBLIC_PI_TOKEN_CONTRACT || 'CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC',
}: EscrowCardProps) {
  ...
  const handleVerifyAndSettle = async () => {
    if (!escrow) return;
    setVerifying(true);
    setVerifyStatus('Generating E-Network biometric proof verification...');
  ...
  return (
    <div className="w-full max-w-[384px] mx-auto rounded-2xl bg-zinc-950 border border-zinc-800/80 p-4 font-mono text-xs text-zinc-200 shadow-2xl space-y-4">
      ...
      <div className="flex space-x-2">
        <input
          ...
          className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-amber-500/50"
        />
        <button
          onClick={() => fetchVault(escrowId)}
          disabled={loading || verifying}
          className="px-3 py-2 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-200 rounded-lg transition"
        >
          SYNC
        </button>
      </div>
      ...
      {escrow ? (
            ...
            <div className="space-y-2 pt-2 border-t border-zinc-800/60">
              {!isExpired && (
                <button
                  ...
                  disabled={loading || verifying}
                  className="w-full py-2.5 bg-linear-to-r from-amber-600/30 to-amber-500/20 hover:from-amber-600/40 text-amber-300 border border-amber-500/40 rounded-lg font-semibold transition text-[11px]"
                >
                  {verifying ? 'VERIFYING BIOMETRICS...' : '⚡ AUTHENTICATE & RELEASE'}
                </button>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  disabled={loading || verifying}
                  className="w-full py-2 bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/30 rounded-lg font-semibold transition text-[11px]"
                >
                  DISPUTE
                </button>
          ...
          {escrow.status === 'Disputed' && (
            <div className="p-2.5 rounded-lg bg-rose-950/20 border border-rose-900/40 text-center">
              ...
              <span className="text-[10px] text-zinc-400 mt-0.5 block">
                Settlement locked pending 5-Elder VRF adjudication[cite: 1].
              </span>
            </div>
          )}
        </div>
        ...
        !loading && (
          <div className="text-center py-6 text-zinc-600 text-xs">
            No active escrow record loaded. Enter an ID to synchronize.
          </div>
        )
      )}
      ...
      {(loading || verifying) && (
        <div className="text-center text-amber-400 text-[11px] animate-pulse">
          Querying Soroban State Machine...
        </div>
      )}
    </div>
  );
