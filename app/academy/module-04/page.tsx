...
export default function GenesisCapstonePage() {
  ...
  // 🛡️ LOGIC PURITY: The live minting function (HARDENED MESH)
  const initiateGenesisMint = async () => {
    if (mintStatus !== "IDLE" && mintStatus !== "FAILED") return;
    
    // SHIELD 1: Pre-Flight Verification
    if (!pioneer || !pioneer.username || pioneer.username === "UNVERIFIED_NODE") {
      setMintStatus("FAILED");
      setLogs([
        "[ADJUDICATOR] FATAL: Node is unverified.",
        "[ADJUDICATOR] Cannot anchor ghost nodes to the Genesis Ledger.",
        "Action: Load this route inside the Pi Browser or Sandbox to authenticate."
      ]);
      return; // Halt execution before hitting MongoDB
    }

    setMintStatus("SIGNING");
    setLogs([`[VAULT] Requesting Genesis Node signature for @${pioneer.username}...`]);
