// Location: app/components/TreasuryMonitor.tsx
"use client";

import React, { useState, useEffect } from "react";

// 🛡️ PROTOCOL BASELINE CONFIGURATION (mBZR Denomination)
const DEFAULTS = {
  CIRCULATION: 100_000_000,
  TREASURY_BASE: 25_000_000,
};
    ...
    <div className="p-5 border-2 border-green-900/40 bg-slate-950 rounded-2xl shadow-[0_0_30px_rgba(34,197,94,0.05)] max-w-86 mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="space-y-1">
          <h2 className="text-[10px] font-bold tracking-[0.3em] text-green-500 uppercase">
            Community Treasury
          </h2>
          <p className="text-[8px] text-slate-500 uppercase tracking-tighter italic">
            Protocol-Owned Liquidity (1:1 Peg)
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-white font-mono tracking-tighter">
            {loading ? (
              ...
              <>
                {metrics.treasuryMBZR.toLocaleString()}{" "}
                <span className="text-green-500 text-xs">mBZR</span>
              </>
            )}
          </p>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-900">
        ...
        <div className="mt-4 p-2 bg-black/40 border border-emerald-950/60 rounded-lg flex justify-between items-center font-mono text-[6px]">
          ...
          <span className="text-emerald-400 truncate max-w-40">
            {loading ? "COMPUTING..." : metrics.proofChecksum}
          </span>
        </div>
