...
export default function CitizenSandboxPortal() {
  // 🛡️ AUTH CONTEXT HOOK: Pulls Pioneer profile without mutating persistent state
  const { pioneer } = useAuth();
  
  // 🛡️ LOCAL TELEMETRY STATE: Stores read-only Horizon / Soroban RPC metrics
  const [telemetry, setTelemetry] = useState<TelemetryData | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  
  // 🛡️ INTERACTION STATE MACHINE: Controls mock claim lifecycle (idle -> processing -> success)
  const [claimState, setClaimState] = useState<"idle" | "processing" | "success">("idle");

  useEffect(() => {
    // 🛡️ READ-ONLY TELEMETRY: Fetch Soroban RPC metrics via public mesh-scan
  ...
  // 🛡️ MOCK FAUCET CLAIM: Simulates on-chain latency without gas deduction
  const handleMockFaucetClaim = () => {
    setClaimState("processing");
    // Simulate network block confirmation delay (2000ms)
    setTimeout(() => {
      setClaimState("success");
    }, 2000);
  };

  // 🛡️ HUD LOADER SHIELD: Rendered during initial mount and telemetry query
  if (isLoading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-mono pb-24">
      {/* 🛡️ S23 ULTRA VIEWPORT CONSTRAINT: Locks width to 384px safe frame */}
      <div className="w-full max-w-sm mx-auto p-4 space-y-6">
        
        {/* HEADER BLOCK: Identity & Sandbox Mode Indicator */}
        ...
        {/* ⚠️ ISOLATION BANNER: Confirms sandbox quarantine from mainnet TrustScore */}
        ...
        {/* 🌐 LIVE HORIZON / SOROBAN TELEMETRY DISPLAY */}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-emerald-400">
                {/* 🛡️ SYNTAX REPAIR: Formats ledger sequence number safely */}
                {telemetry?.latest_ledger ? Number(telemetry.latest_ledger).toLocaleString() : "---"}
              </p>
            </div>
          </div>
        </section>

        {/* 🚰 MOCK TESTNET FAUCET MODULE */}
        ...
        {/* 🛣️ PROGRESSION GATEWAY: Visual roadmap to Genesis Tier */}
        <section className="space-y-3">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-3 space-y-3 relative before:absolute before:inset-y-4 before:left-5.25 before:w-0.5 before:bg-slate-800">
            ...
            {/* Step 2: Sandbox Verification (Current Active Step) */}
            <div className="flex items-center gap-3 relative z-10">
            ...
            {/* Step 3: Pioneer Genesis Gate (Requires Elder Quorum) */}
            <div className="flex items-center gap-3 relative z-10">
