...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // 1. The Fixed Genesis Guard
    const pioneerUid = pioneer?.uid;
    
    if (!pioneerUid) {
      console.warn("[MESH BUFFER] Awaiting Pioneer Identity Handshake...");
      // CRITICAL FIX: Drop the loading shield so the PioneerAuthGate can render
      setIsLoading(false);
      return;
    }

    // Reset loading state now that identity is confirmed and fetch is starting
    setIsLoading(true);
