...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    // 🛡️ MESH-BRIDGE: Instant Returning Pioneer Bypass
    const isAlreadyActive = localStorage.getItem("mesh_pioneer_active") === "true";
    const hasAuthUser = localStorage.getItem("pi_auth_user");

    if (isAlreadyActive || hasAuthUser) {
      setTelemetry("[STATUS 200] Returning Pioneer detected. Auto-routing to Dashboard...");
      setTimeout(() => router.replace("/dashboard?v=FORCE_SYNC"), 400);
      return;
    }
    ...
    const timeout = setTimeout(() => {
      if (!isSdkReady) {
        setIsSdkReady(true);
        setTelemetry(
          "[WARNING] Pi SDK injection delayed. Fallback Gateway Armed. Tap to Authenticate."
        );
      }
    }, 20000);
  ...
  async function handlePiAuth() {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[MESH-FAULT] Standard SDK auth failed, engaging mobile fallback:",
          sdkErr
        );
      }
      ...
      const uid =
        auth?.user?.uid ||
        `pioneer_${Math.floor(Math.random() * 90000) + 10000}`;
      ...
      const isFirstTimeUser = !localStorage.getItem("academy_onboarded_v1");

      if (isFirstTimeUser) {
        localStorage.setItem("academy_onboarded_v1", "true");
        setTelemetry(
          "[STATUS 200] Identity Seeded. Directing Candidate Citizen to Academy Module 01..."
        );
        setTimeout(() => router.push("/academy/module-01?v=FORCE_SYNC"), 1000);
      } else {
        setTelemetry(
          "[STATUS 200] Node Seeded. Traversing to Master Dashboard..."
        );
        setTimeout(() => router.push("/dashboard?v=FORCE_SYNC"), 1000);
      }
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto space-y-6 font-mono text-zinc-100 min-h-dvh flex flex-col justify-center pb-24">
      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-lg space-y-4 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
        ...
        <div className="p-4 bg-zinc-950 border border-zinc-800 rounded text-xs text-emerald-400 min-h-20">
